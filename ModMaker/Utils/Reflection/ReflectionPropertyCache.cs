...
namespace ModMaker.Utils
{
    public static partial class ReflectionCache
    {
        ...
        {
            object cache = null;
            if (_propertieCache.TryGetValue(typeof(T), name, out WeakReference weakRef))
        ...
        {
            object cache = null;
            if (_propertieCache.TryGetValue(type, name, out WeakReference weakRef))
                cache = weakRef.Target;
            if (cache == null)
            {
                cache =
        ...
        public static void SetPropertyValue<T, TProperty>(this ref T instance, string name, TProperty value) where T : struct
        {
        ...
        private class CachedPropertyOfStruct<T, TProperty> : CachedProperty<TProperty>
        {
            ...
            {
                return (_getter ?? (_getter = CreateGetter(typeof(Getter), Info.GetMethod, true) as Getter))(ref _dummy);
            }

            public TProperty Get(ref T instance)
            {
                return (_getter ?? (_getter = CreateGetter(typeof(Getter), Info.GetMethod, true) as Getter))(ref instance);
            }

            public override void Set(TProperty value)
            {
                (_setter ?? (_setter = CreateSetter(typeof(Setter), Info.SetMethod, true) as Setter))(ref _dummy, value);
            }

            public void Set(ref T instance, TProperty value)
            {
                (_setter ?? (_setter = CreateSetter(typeof(Setter), Info.SetMethod, true) as Setter))(ref instance, value);
            }
        }
        ...
        private class CachedPropertyOfClass<T, TProperty> : CachedProperty<TProperty>
        {
            ...
            public override TProperty Get()
            {
                return (_getter ?? (_getter = CreateGetter(typeof(Getter), Info.GetMethod, false) as Getter))(_dummy);
            }

            public TProperty Get(T instance)
            {
                return (_getter ?? (_getter = CreateGetter(typeof(Getter), Info.GetMethod, false) as Getter))(instance);
            }

            public override void Set(TProperty value)
            {
                (_setter ?? (_setter = CreateSetter(typeof(Setter), Info.SetMethod, false) as Setter))(_dummy, value);
            }

            public void Set(T instance, TProperty value)
            {
                (_setter ?? (_setter = CreateSetter(typeof(Setter), Info.SetMethod, false) as Setter))(instance, value);
            }
        }
